...
public class QuadTree implements QuadTreeInterface {
  ...
  private QuadTreeNode addHelper(QuadTreeNode node, Object2D elem) {
    // TODO: Implement this method for Question 2
    if (!node.isLeaf()) {
      addHelper(findSubNode(node, elem), elem);
    } else {
      node.subdivide();
      node.values.add(node.values.size() + 1, elem);
      while (!node.values.isEmpty()) {
        Object2D elementToAdd = node.values.get(1);
        node.values.remove(1);
        QuadTreeNode nodeToAddTo = findSubNode(node, elementToAdd);
        nodeToAddTo.values.add(nodeToAddTo.values.size() + 1, elementToAdd);
      }
    }
    return null;
  }

  private QuadTreeNode findSubNode(QuadTreeNode node, Object2D elem) {
  ...
  private void queryRegionHelper(QuadTreeNode node, AABB region,
    ...
    if(node.isLeaf()){
      ...
      if(overlap(region,node.NE.region)){
        queryRegionHelper(node.NE,region,bucket);
      }
      if(overlap(region,node.NW.region)){
        queryRegionHelper(node.NW,region,bucket);
      }
      if(overlap(region,node.SE.region)){
        queryRegionHelper(node.SE,region,bucket);
      }
      if(overlap(region,node.SW.region)){
        queryRegionHelper(node.SW,region,bucket);
      }
    }
  }

  private boolean overlap(AABB a,AABB b){
    return b.covers(a.topLeft()) || b.covers(a.topRight()) || b.covers(a
        .bottomLeft()) || b.covers(a.bottomRight()) || a.covers(b.topLeft())
        || a.covers(b.topRight()) || a.covers(b
        .bottomLeft()) || a.covers(b.bottomRight());
  }
  /**
